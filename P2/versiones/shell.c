...
void cmdAsignar(int argc, char *argv[]){
    ...
    //printf("%d %d %d %d \n",op_malloc, op_mmap, op_crearshared, op_shared );
...
void cmdDesasignar(int argc, char * argv[]){
    ...
    if (argc>=2){
           ...
           if(dato!=NULL){
               if((s=borrarElemento(LP2, *dato))==-1){
...
void cmdVolcar(int argc, char *argv[]){
	...
	if(argc==3)
	cont=atoi(argv[2]);
	printf("Volcados %d bytes en\n",cont);
}


void recursivaAux(int n){
   char automatico[MAXLINEA];
   static char estatico[MAXLINEA];
